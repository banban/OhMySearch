...
function findSharedImages(lat, lng) {
    ...
    //lat = -35;
    //lng = 138;
    //console.log('@Url.ContentAbsUrl("~/api/")Geo/GetNearestFiles/?lat=' + lat + '&lng=' + lng + '&maxDistance=' + Math.round(searchCircle.radius) + '&topCount=100');
    $.getJSON(baseUrl + 'api/Geo?lat=' + lat + '&lng=' + lng + '&maxDistance=' + Math.round(searchCircle.radius) + '&topCount=100', function (data) {
        ...
        $.each(data, function (i, item) {
            ...
            var marker = new google.maps.Marker({
                ...
                map: map,
                icon: baseUrl + "Content/Images/icon46.png", // http://maps.google.com/mapfiles/kml/pal4/icon46.png "http://maps.google.com/mapfiles/marker_purple.png",
                //shape: shape,
                body: item.body,
                id: item.id
                //,zIndex: beach[3]
            ...
            google.maps.event.addListener(marker, 'click', function () {
                infowindow.setContent('<a title="navigate to file details" href="@Url.ContentAbsUrl("~/")File/Details/?schema=shared&itemId=' + marker.id + '&queryId=0">Details:' + marker.id + '</a><br/>'
                    + marker.body);
...
function LoadInitialPosition(position) {
    infowindow = new google.maps.InfoWindow({
        content: '<div id="infocontent" >You are here.<br/>Drag marker where you need.<br/><button type="button" onclick="findSharedImages(' + position.lat().toFixed(6) + ',' + position.lng().toFixed(6) + ')">Find images</button></div>'
        //content: '<div id="infocontent" >You are here.<br/>Drag marker where you need.<br/><button type="button" onclick="findSharedImages(' + position.H.toFixed(6) + ',' + position.L.toFixed(6) + ')">Find images</button></div>'
    });
        ...
        geocoder.geocode({ 'latLng': position }, function (results, status) {
            ...
            if (status == google.maps.GeocoderStatus.OK) {
                if (results[0]) {
                    ...
                    searchText += "<br/><button type='button' onclick='findSharedImages(" + position.lat().toFixed(6) + "," + position.lng().toFixed(6) + ")'>Find images</button>";
                    //searchText += "<br/><button type='button' id='btnFindSharedImages'>Find images</button>";
...
$(document).ready(function () {
    ...
    $("#MapsTab").click(function () {
        if (map == null) {
            ...
            geocoder.geocode({ 'address': $('#QueryString').val() }, function (results, status) {
                if (status == google.maps.GeocoderStatus.OK) {
            ...
            /*$("#btnFindSharedImages").click(function (event) {
                ...
                $('#QueryString').val(searchText);
                $('#QueryLabel').text(searchText);
                $('#QueryLabel').show();
                $('#QueryLabel').focus();
            });*/
