...
namespace Search.Core.Controllers
{
    public class QueryController : Controller
    {
        ...
        public async Task<IActionResult> Index(string term, string options, string aggregations, int? from, int? page, int? size, double? minScore)
        {
            ...
            foreach (var aggr in response.Aggregations) //.Aggregations
            {
                try
                {
                    var buckets = response.Aggregations.Terms(aggr.Key).Buckets.Where(bct => bct.DocCount > 0)
                ...
                try
                {
                    var buckets2 = response.Aggregations.DateHistogram(aggr.Key).Buckets.Where(bct => bct.DocCount > 0)
                        .OrderByDescending(bct => bct.DocCount) //.ThenBy(bct => bct.KeyAsString)
                        .Take(10);
                ...
                try
                {
                    var buckets3 = response.Aggregations.Range(aggr.Key).Buckets.Where(bct => bct.DocCount > 0)
        ...
        public async Task<List<Models.QueryOption>> GetQueryOptions(string chosenOptions)
        {
            ...
            if (!_memoryCache.TryGetValue("queryOptions", out options))
            {
                ...
                foreach (var item in indexes)
                {
                    ...
                    string prefix = ((alias.Value as AliasDefinition) != null ? alias.Value.ToString() : item.Index);
                    options.Add(new Models.QueryOption()
                    {
                        OptionGroup = "Indices",
                        Key = "1_" + item.Index,
                        Value = prefix + " (" + (string.IsNullOrEmpty(item.DocsCount) ? "0" : item.DocsCount) + " docs" + creationDateLabel + ")"
        ...
        public async Task<Nest.ISearchResponse<dynamic>> GetSearchResponse(Models.Query query)
        {
            ...
            if (query.ChosenOptions != null && query.ChosenOptions.Contains("1_"))
            {
                ...
                indices = Nest.Indices.Index(names);
            ...
            {
                string indexRecords = "";
                if (_memoryCache.TryGetValue("AllIndices", out indexRecords) && !string.IsNullOrEmpty(indexRecords))
                {
            ...
            if (query.ChosenOptions != null && query.ChosenOptions.Contains("2_"))
            {
                var names = query.ChosenOptions.Replace("+", " ").Trim().Split(' ').AsEnumerable()
                    ...
                    .Select(qo => qo.Substring(2).Trim());
                types = Nest.Types.Type(names);//.Select(s => new Nest.TypeName(s))
                foreach (var name in names)
                {
                    keyIndexType += name + ",";
                }
                        ...
                        && query.ChosenOptions != null && query.ChosenOptions.Contains("3_6"))
            {
                ...
                if (!_memoryCache.TryGetValue("FieldsForMLT:" + fullId[0] + "/" + fullId[1], out fields))
                {
                    ...
                    {
                        foreach (var typeMapping in index.Value.Mappings)
                        {
                            if (typeMapping.Key != null && typeMapping.Value != null && typeMapping.Value.Properties != null)
                            {
                                foreach (var fieldMapping in typeMapping.Value.Properties)
                                {
                                    if (fieldMapping.Value != null 
                                            &&  (fieldMapping.Value.Type == "text" 
                                                || fieldMapping.Value.Type == "keyword" 
                                                || fieldMapping.Value.Type == "string")
                                            )
                                    {
            ...
            if (query.ChosenOptions != null && query.ChosenOptions.Contains("5_1"))
            {
                ...
                if (!_memoryCache.TryGetValue(key, out termList))
                {
                    ...
                    foreach (var index in mapping.Indices)
                    {
                        ...
                        {
                            if (index.Key != null && index.Value != null)
                            {
                                foreach (var fieldMapping in index.Value.Mappings)
                                {
                                    //var textValue = (fieldMapping.Value as Nest.TypeMapping(Nest.TextProperty));
                                    if (fieldMapping.Value != null && !termList.Contains(fieldMapping.Key.Name)
                                        ...
                                        && (
                                            fieldMapping.Value.GetType().Name == "keyword" //can't use text fields for terms aggregation
                                            || (fieldMapping.Value.Meta.Where(f => f.Key == "keyword") != null) //some text fields are keyword in mind
                                            )
                                        )
                                    {
            ...
            if (query.ChosenOptions != null && query.ChosenOptions.Contains("5_2"))
            {
                ...
                if (!_memoryCache.TryGetValue(key, out termList))
                {
                    ...
                    foreach (var index in mapping.Indices)
                    {
                        foreach (var fieldMapping in index.Value.Mappings)
                        {
                            if (index.Key != null && index.Value != null) //&& index.Value.Properties != null
                            {
                                //foreach (var fieldMapping in typeMapping.Properties)
                                {
                                    if (fieldMapping.Value != null && !termList.Contains(fieldMapping.Key.Name) 
                                        && fieldMapping.Value.GetType().Name == "date")
                                    {
                                        termList.Add(fieldMapping.Key.Name);
                                    }
                                }
                    ...
                    foreach (var index in mapping.Indices)
                    {
                        foreach (var typeMapping in index.Value.Mappings)
                        {
                            if (typeMapping.Key != null && typeMapping.Value != null && typeMapping.Value.Properties != null)
                            {
                                ...
                                {
                                    if (fieldMapping.Value != null && !termList.Contains(fieldMapping.Key.Name)
                                        && (fieldMapping.Value.GetType().Name == "double" 
                                            || fieldMapping.Value.GetType().Name == "float" 
                                            || fieldMapping.Value.GetType().Name == "number" 
                                            || fieldMapping.Value.GetType().Name == "integer" 
                                            || fieldMapping.Value.GetType().Name == "short" 
                                            || fieldMapping.Value.GetType().Name == "long")
                                        )
                                    {
                                        termList.Add(fieldMapping.Key.Name);
                                    }
                ...
                if (stats.Count > 0)
                {
                    ...
                    if (statsResponse.IsValid)
                    {
                        foreach (var agg in statsResponse.Aggregations)
                        {
                            if (agg.Value != null)
                            {
                                var minValue = statsResponse.Aggregations.Stats(agg.Key).Min;
                                var maxValue = statsResponse.Aggregations.Stats(agg.Key).Max;
        ...
        public List<Models.SearchResult> GetSearchResults(string userName, Nest.ISearchResponse<dynamic> nestResults, string queryTerm)
        {
            ...
            foreach (var hit in nestResults.Hits)
            {
                ...
                Models.SearchResult result = new Models.SearchResult()
                {
                    ...
                    //LastModified = (DateTime)hit.SourceLastModified,
